...
router.post('/', async (req, res) => {
    ...
    const { date, workout, duration, distance} = req.body.formData;
    ...
    try {
      ...
      user.endurances.push(newWorkout);
      await user.save();
  ...
  router.get('/', async (req, res) => {
    ...
    try {
      ...
      res.json(user.endurances); // Return only the workouts of the authenticated user
    } catch (error) {
      res.status(500).json({ error: 'An error occurred. Please try again.' });
    }
  });
  ...
  router.get('/getWorkout/:id', async (req, res) => {
    ...
    try {
      ...
      const workout = user.endurances.id(workoutId);
      if (!workout) {
  ...
  router.put('/updateWorkout/:id', async (req, res) => {
    ...
    try {
      ...
      const workoutToUpdate = user.endurances.id(workoutId);
      if (!workoutToUpdate) {
        return res.status(404).json({ error: 'Workout not found' });
      }
  ...
  router.delete('/deleteWorkout/:id', async (req, res) => {
    ...
    try {
      ...
      const workoutIndex = user.endurances.findIndex(workout => workout._id.toString() === workoutId);
      if (workoutIndex === -1) {
        return res.status(404).json({ error: 'Workout not found' });
      }
  
      user.endurances.splice(workoutIndex, 1);
      await user.save();
